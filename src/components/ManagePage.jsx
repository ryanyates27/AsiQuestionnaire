...
export default function ManagePage({ onBack }) {
  ...
  const refresh = async () => {
    const data = await window.api.getManageQuestions({
      query,
      status: filterStatus,
    });
    setResults(data);
  ...
  useEffect(() => {
    autoResize(questionEditRef.current);
  }, [editingId, editVals.question]);
  useEffect(() => {
    autoResize(answerEditRef.current);
  }, [editingId, editVals.answer]);
  useEffect(() => {
    if (editingInfo) autoResize(infoEditRef.current);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (window.api?.sync?.onState) {
        unsub = window.api.sync.onState(
          (s) => s?.phase && setSyncPhase(s.phase),
        );
      }
    })();
    return () => {
  ...
  const deleteSelected = async () => {
    ...
    try {
      await Promise.all(
        [...selectedIds].map((id) => window.api.removeQuestion(id)),
      );
      setSelectedIds(new Set());
  ...
  const onPublish = async () => {
    if (offline) {
      setPublishMsg(
        "Offline: cannot publish to server until connection is restored.",
      );
      setPublishErrs(null);
      return;
    }
    ...
    try {
      ...
      if (!res?.ok) {
        if (Array.isArray(res?.conflicts) && res.conflicts.length) {
          ...
          setPublishMsg(
            `Conflicts: ${res.conflicts.length} record(s) changed on server since your last sync.`,
          );
        } else {
        ...
        setPublishMsg(
          `Published. Created: ${res.created}, Updated: ${res.updated}, Soft-deleted: ${res.deleted}.`,
        );
      }
    } catch (e) {
    ...
    <>
      <PageWrapper onBack={onBack} title="Manage Questions">
            ...
            <button
              ...
              title={
                offline
                  ...
                  : "Publish local changes to PocketBase"
              }
              style={{
                display: "inline-flex",
                alignItems: "center",
            ...
            {Array.isArray(publishErrs) && publishErrs.length > 0 && (
              <div style={{ marginTop: 6, fontSize: 13 }}>
                ...
                <div style={{ marginTop: 4 }}>
                  Tip: pull latest (restart or retry sync on login), review,
                  then publish again.
                </div>
              </div>
            )}
        ...
        >
          ...
          >
            <colgroup>
              <col style={{ width: "10px" }} />                             {/* Checkbox column sizing */}
              <col style={{ width: questionColWidth }} />                   {/* Question column sizing */}
              <col style={{ width: answerColWidth }} />                     {/* Answer column sizing */}       
              <col style={{ width: "15px" }} />                             {/* Info column sizing */}
              {showUnapprovedLayout && <col style={{ width: "18px" }} />}   {/* Approve column sizing */}
              {showApprovedTagColumn && <col style={{ width: "18px" }} />}  {/* Tag edit column sizing */}
              <col style={{ width: "12px" }} />                             {/* Edit/Save column sizing */}
            </colgroup>
            {Object.entries(grouped).map(([tag, subs]) => (
              <tbody key={tag}>
                <tr>
                  <td
                    colSpan={
                      showUnapprovedLayout ? 6 : showApprovedTagColumn ? 6 : 5
                    }
                    style={{
                      background: "#000",
                      color: "#fff",
        ...
        {tagModal && (
          ...
          >
            ...
            >
              ...
              <p style={{ marginTop: 0, color: "#444" }}>{tagModal.question}</p>
                  ...
                  {(subtypeOptions[tagDraft.tag] || []).map((sub) => (
                    ...
                    >
                      <input
                        ...
                        onChange={() =>
                          setTagDraft((prev) => ({ ...prev, subtag: sub }))
                        }
                        style={{ marginRight: 4 }}
                      />
                      {sub}
